<?php 
	...
	if ( $ListID=="0") {
		$query = mysql_query('SELECT * FROM todolists WHERE userID='.$_SESSION["UserID"].' ORDER BY ID DESC');
	...
	$query = mysql_query('SELECT * FROM todoitems WHERE userID=1 and ListID='.$_SESSION["ListID"].' AND ListVersion='.$ListVersion.' ORDER BY ItemID ASC');
	$unsorted = array();
	...
	echo '<script type="text/javascript"> $("#todo-list-title").text("'.$ListName.'"); ';
